...
struct OneshotDnsResponse<T>(oneshot::Sender<DnsResponseStream<T>>);

impl<T> OneshotDnsResponse<T> {
    fn send_response(
        self,
        serial_response: DnsResponseStream<T>,
    ) -> Result<(), DnsResponseStream<T>> {
        self.0.send(serial_response)
    }
}

/// A Stream that wraps a oneshot::Receiver<Stream> and resolves to items in the inner Stream
pub enum DnsResponseReceiver<T> {
    /// The receiver
    Receiver(oneshot::Receiver<DnsResponseStream<T>>),
    /// The stream once received
    Received(DnsResponseStream<T>),
